  ...
  ui(new Ui::MainWindow)
{
  ...
  QFontMetrics FM(f);
  for (;;){
      wid=FM.width("Project");
      he=FM.height();
      if ((38*ui->menuokok->width()/91>=wid && 12*ui->menuokok->height()/17>=he) || f.pointSize()==1 ){
          break;
        }
  ...
  MainObject= new Menu (this->width(), this->height()-ui->menuBar->height(), 0, ui->menuBar->height());
  ...
  MainObject->child[0]->addChildren(new BeginBlock(0.85, 0.065, 0.06, 0.06, MainObject->child[0], this));
  MainObject->child[0]->addChildren(new EndBlock(0.85, 0.065, 0.06, 0.145, MainObject->child[0], this));
  MainObject->child[0]->addChildren(new DataInputBlock(0.85, 0.065, 0.06, 0.29, MainObject->child[0], this));
  MainObject->child[0]->addChildren(new DataOutputBlock(0.85, 0.065, 0.06, 0.375, MainObject->child[0], this));
  MainObject->child[0]->addChildren(new ArithmeticBlock(0.85, 0.065, 0.06, 0.46, MainObject->child[0], this));
  MainObject->child[0]->addChildren(new IfBlock(0.85, 0.065, 0.06, 0.605, MainObject->child[0], this));
  MainObject->child[0]->addChildren(new WhileBlock(0.85, 0.065, 0.06, 0.69, MainObject->child[0], this));
...
void MainWindow::mousePressEvent(QMouseEvent *event){
  if (!FukingActiveSlot){
    this->TempArea.setX(c.pos().x()-this->geometry().left()); this->TempArea.setY(c.pos().y()-this->geometry().top());
    //if (SelectObject!=nullptr &&)
    if (SelectObject!=nullptr && SelectObject->IsBlock && SelectObject->child[0]->whoIsDaddy(&TempArea)==nullptr && SelectObject->activate && SelectObject->child[1]->whoIsDaddy(&TempArea)!=nullptr){

    }
    else{
        if (SelectObject!=nullptr && SelectObject->IsBlock && SelectObject->activate && SelectObject->whoIsDaddy(&TempArea)==nullptr){
      ...
      if (SelectObject!=MainObject){
          ...
          if (SelectObject->IsBlock && SelectObject->parent->TypeELEM=="LeftMenu"){
              if (SelectObject->TypeELEM=="Begin"){
                  SelectObject->parent->addChildren(new BeginBlock(0.85, 0.065, 0.06, 0.06, MainObject->child[0], this));
          ...
          SelectObject->parent->child.erase(std::find(SelectObject->parent->child.begin(), SelectObject->parent->child.end(), SelectObject));
...
void MainWindow::RebuildThis(Workspace *AnyWorkSpace){
 // QMessageBox::critical(this, "Way of Process", "Begin RebuildThis");
  ...
  for (int i=0; i<AnyWorkSpace->child.size(); i++){
    AnyWorkSpace->child[i]->moveToHARD((double)((AnyWorkSpace->width-AnyWorkSpace->child[i]->width)/2+AnyWorkSpace->left), PointY*AnyWorkSpace->heigth+AnyWorkSpace->top);
    PointY+=AnyWorkSpace->child[i]->kofHeigth+0.02;
  }
